...
cd ~/j5
git fetch origin
git checkout origin/race-manager -- scripts/run_racemanager.sh
grep -n "--doctor" scripts/run_racemanager.sh
```

Or run launcher self-identification to confirm you are executing the updated script:

```bash
./scripts/run_racemanager.sh --version
```
