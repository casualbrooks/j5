...
_SETUP_STEPS = [
    ...
    },
    {
        ...
        "check_commands": ["GET {preview_url}/ready or /health"],
        ...
        "help": "Start the preview service to restore the Computer Vision feed, then verify {preview_url}/ready or {preview_url}/health is reachable.",
    },
    {
...
async def _build_setup_status():
    ...
    for step in _SETUP_STEPS:
        ...
        if step["id"] == "race_state":
            ...
            checks = [
                await _check_http_endpoints(
                    config["preview_url"], ["/ready", "/health"]
                )
            ]
            check_ok = all(item["ok"] for item in checks)
