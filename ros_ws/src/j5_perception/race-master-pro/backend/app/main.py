...
_SETUP_STEPS = [
    ...
    },
    {
        ...
        "check_commands": ["GET {preview_url}/health or /snapshot.jpg or /stream.mjpg"],
        ...
        "help": "Start the preview service to restore the Computer Vision feed, then verify at least one of {preview_url}/health, /snapshot.jpg, or /stream.mjpg is reachable.",
    },
    {
...
async def _build_setup_status():
    ...
    for step in _SETUP_STEPS:
        ...
        if step["id"] == "race_state":
            ...
            checks = [
                await _check_http_endpoints(
                    config["preview_url"], ["/health", "/snapshot.jpg", "/stream.mjpg"]
                )
            ]
            check_ok = all(item["ok"] for item in checks)
